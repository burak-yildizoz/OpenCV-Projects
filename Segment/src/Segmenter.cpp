...
Segmenter::Segmenter(const cv::Mat &img, float sigma, float k, int min_size)
  ...
  for (int x = 0; x < _labelImage.cols; x++) {
    for (int y = 0; y < _labelImage.rows; y++) {
      ...
      if (std::find(_labelvec.begin(), _labelvec.end(), label) ==
          _labelvec.end())
        // the label is new
        _labelvec.push_back(label);
      uchar id = (uchar)std::distance(
          _labelvec.begin(),
          std::find(_labelvec.begin(), _labelvec.end(), label));
...
cv::Mat Segmenter::getLabelImage() const { return _labelImage.clone(); }
...
std::vector<Contour> Segmenter::contoursOfLabels() const {
  ...
  for (int i = 0; i < _num_ccs; i++) {
    ...
    cv::findContours((cv::Mat)(_labelImage == i), contours, cv::RETR_LIST,
