...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    std::vector<bool> status =
        Stitcher::findHomography(matches.first, matches.second,
                                 lastFeature.getKeypoints(),
    ...
    std::vector<std::vector<int>> featuresOfPrevSegments(
    ...
    for (int i = 0; i < static_cast<int>(featuresOfSegments.size()); i++) {
      ...
      if (featuresOfSegments[i].size()) {
        std::pair<int, int> occurrence =
            general::most_frequent_element(featuresOfSegments[i]);
        int prevID = occurrence.first;
        // make sure it is also matched the other way around
        std::pair<int, int> lastOccurrence =
            general::most_frequent_element(featuresOfPrevSegments[prevID]);
    ...
    static std::vector<cv::Vec3b> prevColors(
        rainbow.begin(),
        rainbow.begin() + lastSegment.numSegments); // last label colors
    std::vector<bool> colorUsed(256, false);        // currently used colors
    std::vector<cv::Vec3b> currColors(segment.numSegments,
                                      cv::Vec3b(0, 0, 0)); // label colors
    ...
    for (size_t i = 0; i < prevIDs.size(); i++) {
      ...
      colorUsed[std::distance(
          rainbow.begin(), std::find(rainbow.begin(), rainbow.end(), color))] =
          true;
    }
    ...
    for (cv::Vec3b &color : currColors) {
      if (color == cv::Vec3b(0, 0, 0)) {
        size_t index =
            std::distance(colorUsed.begin(),
                          std::find(colorUsed.begin(), colorUsed.end(), false));
        color = rainbow[index];
    ...
    labelImage.forEach<uchar>(
        [&currColors, &disp](uchar label, const int pos[2]) {
    ...
    ci.connectCenters(compFeature, trainIDs, queryIDs, lastFeature.kps,
                      feature.kps);
    ...
    ci.drawContours(painted, prevContours, currContours, prevIDs, currIDs,
                    prevColors, currColors);
    compTr = imgops::blend(compTr, painted, 0.3);
    ci.drawContours(compTr, prevContours, currContours, prevIDs, currIDs,
                    prevColors, currColors, 1, false);
    ci.connectCenters(compTr, prevContours, currContours, prevIDs, currIDs,
                      currColors, false);
