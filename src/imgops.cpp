...
cv::Mat ConnectImages::compareImages(const cv::Mat &prevImg,
                                     const cv::Mat &nextImg) const {
  cv::Mat compImg = cv::Mat::zeros(
      MAX(_nextRect.height + _nextRect.y, _prevRect.height + _prevRect.y),
      MAX(_nextRect.width + _nextRect.x, _prevRect.width + _prevRect.x),
      CV_8UC3);
...
void ConnectImages::connectCenters(cv::Mat &compImg,
  ...
  for (size_t i = 0; i < sz; i++) {
    ...
    cv::line(compImg, prevCenter + _prevRect.tl(), nextCenter + _nextRect.tl(),
  ...
  if (markCenters) {
    for (size_t i = 0; i < sz; i++) {
      ...
      cv::Point prevCenter = trainKps[trainID].pt;
      cv::Point nextCenter = queryKps[queryID].pt;
      cv::circle(compImg, prevCenter + _prevRect.tl(), 2, CV_RGB(255, 0, 0),
                 cv::FILLED, cv::LINE_AA);
      cv::circle(compImg, nextCenter + _nextRect.tl(), 2, CV_RGB(0, 0, 255),
                 cv::FILLED, cv::LINE_AA);
    }
  }
}
...
void ConnectImages::connectCenters(cv::Mat &compImg,
  ...
  for (size_t i = 0; i < sz; i++) {
    ...
    cv::Vec3b color =
        invertedColor ? (cv::Vec3b::all(255) - origColor) : origColor;
    cv::line(compImg, prevCenter + _prevRect.tl(), nextCenter + _nextRect.tl(),
             color, 1, cv::LINE_AA);
  }
}

void ConnectImages::drawBoxes(
    cv::Mat &compImg, const std::vector<Contour> &prevContours,
    const std::vector<Contour> &nextContours, const std::vector<int> &prevIDs,
    const std::vector<int> &nextIDs, const std::vector<cv::Vec3b> &prevColors,
    const std::vector<cv::Vec3b> &nextColors, bool invertedColor) const {
  ...
  for (size_t i = 0; i < prevIDs.size(); i++) {
    ...
    cv::rectangle(
        compImg,
        cv::Rect(_prevRect.tl() + contourRect.tl(), contourRect.size()), color);
  }
  for (size_t i = 0; i < nextIDs.size(); i++) {
    ...
    cv::rectangle(
        compImg,
        cv::Rect(_nextRect.tl() + contourRect.tl(), contourRect.size()), color);
  }
}
...
void ConnectImages::drawContours(cv::Mat &compImg,
  ...
  for (size_t i = 0; i < prevIDs.size(); i++) {
    ...
    for (size_t i = 0; i < contour.size(); i++)
      contour[i] += tl;
    cv::drawContours(compImg, std::vector<std::vector<cv::Point>>(1, contour),
                     0, color, thickness);
  }
  for (size_t i = 0; i < nextIDs.size(); i++) {
    int nextID = nextIDs[i];
...
cv::Vec3b scalar2vec3b(cv::Scalar s) {
  return cv::Vec3b(static_cast<int>(s[0]), static_cast<int>(s[1]),
                   static_cast<int>(s[2]));
}
} // namespace
...
void ConnectImages::numberPoints(cv::Mat &compImg,
  ...
  for (size_t i = 0; i < prevIDs.size(); i++) {
    ...
    if (!color_provided)
      targetColor = invertedColor ? (cv::Vec3b::all(255) - prevColors[prevID])
                                  : prevColors[prevID];
    std::string num = std::to_string(prevID);
    cv::putText(compImg, num, pt + _prevRect.tl(), cv::FONT_HERSHEY_SIMPLEX,
                0.4, targetColor, 1, cv::LINE_AA);
  }
  for (size_t i = 0; i < nextIDs.size(); i++) {
    int nextID = nextIDs[i];
    ...
    cv::Vec3b targetColor = scalar2vec3b(color);
    if (!color_provided)
      targetColor = invertedColor ? (cv::Vec3b::all(255) - nextColors[nextID])
                                  : nextColors[nextID];
    std::string num = std::to_string(nextID);
    cv::putText(compImg, num, pt + _nextRect.tl(), cv::FONT_HERSHEY_SIMPLEX,
                0.4, targetColor, 1, cv::LINE_AA);
  }
}
