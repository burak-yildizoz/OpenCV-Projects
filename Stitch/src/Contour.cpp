...
Contour::Contour(std::vector<cv::Point> contourPoints)
    : _contourPoints(contourPoints), _m(cv::moments(contourPoints)),
...
double Contour::area() const { return cv::contourArea(_contourPoints); }

cv::Rect Contour::box() const { return cv::boundingRect(_contourPoints); }

cv::Point Contour::weightedCenter() const {
  return cv::Point(static_cast<int>(_m.m10 / _m.m00),
                   static_cast<int>(_m.m01 / _m.m00));
}

std::vector<cv::Point> Contour::contourPoints() const { return _contourPoints; }
...
cv::Mat Contour::getMask() const {
  ...
  if (_isConvex)
    ...
    cv::fillPoly(mask, std::vector<std::vector<cv::Point>>(1, contour),
                 cv::Scalar::all(255));
  return mask;
}
