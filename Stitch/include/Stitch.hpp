...
public:
    ...
    // apply affine transform given homography matrix and the top-left coordinate orig
    // orig is moved to (0, 0) to be covered in the result by applying translation to homography matrix
    ...
    // stitch two images without losing the points that falls outside the first quadrant
    static cv::Mat stitch(const cv::Mat& prevImg, const cv::Mat& nextImg, const cv::Mat& H);
    // paste the previous pano onto the next one and return the corresponding coordinate of the given point
    // the given pixel coordinates must be the origin of the same image
    static std::pair<cv::Mat, cv::Point> patchPano(
