// g++ jpeg_compression.cpp -o jpeg_compression.exe -Wall -Wextra -Wpedantic -O2 -std=c++11 $(pkg-config --libs --cflags opencv)
...
#define DEBUG(cc) if(true) { std::cout << #cc << " = \n"<< cc << "\n\n"; }
...
size_t outputSize(const cv::Mat& B)
{
    ...
    for (int i = 0; i < B.rows; i += 8)
    {
        for (int j = 0; j < B.cols; j += 8)
        {
            ...
            It it = mset.begin();
            std::vector< std::pair<int, int> > probabilities;   // occurance, index of mset
            while (it != mset.end())
            {
            ...
            std::vector< std::pair< int, std::vector<int> > > tree; // occurance, indexes of probabilities
            for (unsigned k = 0; k < probabilities.size(); k++)
            {
                tree.push_back(std::make_pair(probabilities[k].first, std::vector<int>(1, k)));
            }
            while (tree.size() != 1)
            {
                std::sort(tree.begin(), tree.end(), [](const std::pair< int, std::vector<int> >& lhs, const std::pair< int, std::vector<int> >& rhs) { return lhs.first < rhs.first; });
...
void trackbar_callback(int quality, void* image_p)
{
    if (quality == 0)
        quality = 1;
    std::pair<const cv::Mat&, cv::Mat&>* image = (std::pair<const cv::Mat&, cv::Mat&>*)image_p;
    ...
    if (A.type() == CV_8UC1)
    {
        ...
        //cv::resize(Ad, disp_image, cv::Size(400, 400 * Ad.rows / Ad.cols), 0, 0, cv::INTER_AREA);
    }
    else if (A.type() == CV_8UC3)
    {
        ...
        //cv::resize(disp_image, disp_image, cv::Size(400, 400 * disp_image.rows / disp_image.cols), 0, 0, cv::INTER_AREA);
    }
    else assert((A.type() == CV_8UC1) || (A.type() == CV_8UC3));
...
void show_quantization(const cv::Mat& A)
{
    ...
    cv::createTrackbar("quality", winname, &quality, 100, trackbar_callback, (void*)&image);
...
int main(int argc, char* argv[])
{
    ...
    //cv::resize(disp_orig, disp_orig, cv::Size(400, 400 * A.rows / A.cols), 0, 0, cv::INTER_AREA);
